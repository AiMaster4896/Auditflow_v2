...
export const ROUTE_ACCESS = {
  ...
  "/elearn": ["firm_admin", "manager", "staff"],
...
export function canAccess(route) {
  ...
  if (route === "/elearn" && identity.role !== "firm_admin" && !identity.elearnEnabled) return false;
...
export const SIDEBAR_ITEMS = [
  ...
  { route: "/elearn", label: "E-Learn", icon: "graduation-cap", roles: ["firm_admin", "manager", "staff"] },
...
export function visibleSidebarItems() {
  ...
  return SIDEBAR_ITEMS.filter((item) => {
    ...
    if (item.route === "/elearn" && identity.role !== "firm_admin" && !identity.elearnEnabled) return false;
    return true;
  });
}
