...
export async function renderMonitoring(el) {
  ...
  await reloadMonitoringData(orgId);
  populateMonitoringFilters(clientsForFilterCache, teamsForFilterCache, branchesForFilterCache);
...
let clientsForFilterCache = [];
let teamsForFilterCache = [];
let branchesForFilterCache = [];

async function reloadMonitoringData(orgId) {
  orgId = orgId || getIdentity()?.organisationId;
  ...
  clientsForFilterCache = clientsData || [];
  teamsForFilterCache = teamsData || [];
  branchesForFilterCache = branchesData || [];
}
...
async function openMonitoringRow(deadline) {
  ...
  if (deadline.work_order_id) {
    const { data: wo } = await supabase
      ...
      .maybeSingle();
    if (wo) showWorkOrderDetailsModal(wo, async () => { await reloadMonitoringData(); paintMonitoringTable(); });
    return;
  }
