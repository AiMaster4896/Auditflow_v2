...
let tetMonthsRef = [];
let tetListBaseItems = [];
let tetListFilters = { fyeMonth: "", letter: "" };
let tetListLastTitle = null;
...
export async function renderTaxEstimateMonitoring(el) {
  ...
  el.innerHTML = `
    ...
    <div class="import-card">
      <div style="display:flex;align-items:center;gap:12px;margin-bottom:4px;">
        <h3 style="margin:0;" id="tet-list-title">All Clients</h3>
        <button type="button" id="tet-back-to-all" class="btn-link hidden">&larr; Back to All Clients</button>
      </div>
      <div id="tet-list-filter-letter" style="display:flex;gap:4px;flex-wrap:wrap;margin-bottom:12px;"></div>
      <div id="tet-list-filter-month" style="display:flex;gap:4px;flex-wrap:wrap;margin-bottom:14px;"></div>
  ...
  const [{ data: tracking }, { data: deadlines }, { data: clientsData }, { data: teamsData }, { data: branchesData }, { data: contacts }] = await Promise.all([
    ...
    supabase.from("clients").select("id, legal_name, tin, team_id, engagement_type, fye_month").eq("organisation_id", orgId).in("engagement_type", ["tax", "both"]),
  ...
  document.getElementById("tet-branch-filter").addEventListener("change", (e) => {
    ...
    renderChart();
    tetPage = 1;
    renderList(scopedTetItems(), "All Clients");
  });
...
function renderTetLetterFilter() {
  const el = document.getElementById("tet-list-filter-letter");
  if (!el) return;
  const letters = ["All", "123", ..."ABCDEFGHIJKLMNOPQRSTUVWXYZ".split("")];
  el.innerHTML = letters.map((l) => {
    const value = l === "All" ? "" : l;
    const isActive = tetListFilters.letter === value;
    return `<button type="button" class="btn-secondary" data-tet-letter="${value}" style="padding:4px 10px;font-size:12px;${isActive ? "background:var(--gray-900);color:#fff;border-color:var(--gray-900);" : ""}">${l}</button>`;
  }).join("");
  el.querySelectorAll("[data-tet-letter]").forEach((btn) => {
    btn.addEventListener("click", () => {
      tetListFilters.letter = btn.dataset.tetLetter;
      tetPage = 1;
      const title = document.getElementById("tet-list-title").textContent;
      renderList(tetListBaseItems, title);
    });
  });
}

function renderTetMonthFilter() {
  const el = document.getElementById("tet-list-filter-month");
  if (!el) return;
  const monthNames = ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"];
  el.innerHTML = `<button type="button" class="btn-secondary" data-tet-month="" style="padding:4px 10px;font-size:12px;${!tetListFilters.fyeMonth ? "background:var(--gray-900);color:#fff;border-color:var(--gray-900);" : ""}">All</button>` +
    monthNames.map((label, i) => {
      const value = String(i + 1);
      const isActive = tetListFilters.fyeMonth === value;
      return `<button type="button" class="btn-secondary" data-tet-month="${value}" style="padding:4px 10px;font-size:12px;${isActive ? "background:var(--gray-900);color:#fff;border-color:var(--gray-900);" : ""}">${label}</button>`;
    }).join("");
  el.querySelectorAll("[data-tet-month]").forEach((btn) => {
    btn.addEventListener("click", () => {
      tetListFilters.fyeMonth = btn.dataset.tetMonth;
      tetPage = 1;
      const title = document.getElementById("tet-list-title").textContent;
      renderList(tetListBaseItems, title);
    });
...
function renderList(rawItems, title) {
  document.getElementById("tet-list-title").textContent = title;
  tetListBaseItems = rawItems;
  tetSelectedIds.clear();
  if (title !== tetListLastTitle) {
    tetListFilters = { fyeMonth: "", letter: "" };
    tetListLastTitle = title;
  }

  const backBtn = document.getElementById("tet-back-to-all");
  ...
  renderTetMonthFilter();
  renderTetLetterFilter();

  const items = rawItems.filter((t) => {
    if (tetListFilters.fyeMonth && String(t.fye_month) !== tetListFilters.fyeMonth) return false;
    if (tetListFilters.letter) {
      const firstChar = (t.legal_name || "").trim().charAt(0).toUpperCase();
      if (tetListFilters.letter === "123") { if (!/[0-9]/.test(firstChar)) return false; }
      else if (firstChar !== tetListFilters.letter) return false;
    }
    return true;
  });
  ...
  document.getElementById("tet-send-now-btn").addEventListener("click", async () => {
    ...
    try {
      ...
      alert(message);
      const originalIds = tetListBaseItems.map((i) => i.id);
      await loadData();
...
async function openAutomationSetupModal() {
  ...
  backdrop.innerHTML = `
    <div class="modal-card" style="max-width:900px;width:95vw;">
      ...
      <div class="tab-nav" style="display:flex;gap:4px;border-bottom:1px solid var(--gray-200);margin-bottom:20px;">
        <button type="button" class="tet-modal-tab-btn" data-tet-tab="setup">Setup</button>
        <button type="button" class="tet-modal-tab-btn" data-tet-tab="cp204">CP204 Templates</button>
        <button type="button" class="tet-modal-tab-btn" data-tet-tab="cp204a">CP204A Templates</button>
        <button type="button" class="tet-modal-tab-btn" data-tet-tab="test">Send Test</button>
      </div>

      <div class="tet-modal-panel" data-tet-panel="setup">
      ...
      <div class="tet-modal-panel" data-tet-panel="cp204" style="display:none;">
      ...
      <div class="tet-modal-panel" data-tet-panel="cp204a" style="display:none;">
      ...
      <div class="tet-modal-panel" data-tet-panel="test" style="display:none;">
  ...
  function setActiveTetTab(tab) {
    backdrop.querySelectorAll(".tet-modal-tab-btn").forEach((btn) => {
      const isActive = btn.dataset.tetTab === tab;
    ...
    backdrop.querySelectorAll(".tet-modal-panel").forEach((panel) => {
      panel.style.display = panel.dataset.tetPanel === tab ? "" : "none";
    });
  }
  backdrop.querySelectorAll(".tet-modal-tab-btn").forEach((btn) => {
    btn.addEventListener("click", () => setActiveTetTab(btn.dataset.tetTab));
  });
  setActiveTetTab("setup");
