...
async function openDeadlineCellModal(deadline, client, matrixContainer) {
  ...
  if (!admin && deadline.work_order_id) {
    ...
    supabase
      ...
      .maybeSingle()
      .then(({ data: wo }) => { if (wo) showWorkOrderDetailsModal(wo, () => renderComplianceMatrix(matrixContainer, client)); });
    return;
  }

  const isAuditType = deadline.deadline_type === "Audit";
  const isTaxType = ["Form C", "CP204", "CP204A 6th", "CP204A 9th", "CP204A 11th"].includes(deadline.deadline_type);
  const showsEditableDates = isAuditType && !deadline.work_order_id;

  let taxInfo = null;
  if (isTaxType) {
    const orgId = getIdentity()?.organisationId;
    const { data } = await supabase
      .from("tax_estimate_tracking")
      .select("this_year_estimate, final_tax_balance")
      .eq("client_id", client.id)
      .eq("financial_year", deadline.financial_year)
  ...
  backdrop.innerHTML = `
    <div class="modal-card" style="max-width:420px;">
      ...
      ${showsEditableDates ? `
        <div style="border:1px solid var(--gray-200);border-radius:8px;padding:14px;margin-bottom:14px;">
          <label>Audit Report Date
            <input type="text" id="dcm-report-date" placeholder="31/12/2025" autocomplete="off" value="${isoToDMY(deadline.manual_report_date) || ""}" />
          </label>
          <label style="margin-top:10px;">Archival Date
            <input type="text" id="dcm-archival-date" placeholder="31/12/2025" autocomplete="off" value="${isoToDMY(deadline.manual_archival_date) || ""}" />
          </label>
          <button id="dcm-save-dates" class="btn-dark" style="margin-top:10px;">Save Dates</button>
          <p class="hint" style="margin-top:6px;">Filling in both dates automatically marks this Completed.</p>
          <p id="dcm-dates-error" class="form-error hidden"></p>
        </div>
      ` : ""}

      ${isTaxType ? `
        <div style="border:1px solid var(--gray-200);border-radius:8px;padding:14px;margin-bottom:14px;">
          <label>${deadline.deadline_type === "Form C" ? "Final Tax" : deadline.deadline_type === "CP204" ? "Tax Estimate" : "Revision"} Amount (RM)
            <input type="number" step="0.01" id="dcm-tax-amount" value="${(deadline.deadline_type === "Form C" ? taxInfo?.final_tax_balance : taxInfo?.this_year_estimate) ?? ""}" />
          </label>
          <button id="dcm-save-amount" class="btn-dark" style="margin-top:10px;">Save Amount</button>
          <p class="hint" style="margin-top:6px;">Saving an amount automatically marks this Completed.</p>
          <p id="dcm-amount-error" class="form-error hidden"></p>
        </div>
      ` : ""}
      ...
      <div style="display:flex;flex-direction:column;gap:10px;">
        ...
        ${!isAuditType && !isTaxType ? `<button id="dcm-mark-completed" class="btn-secondary">Mark as Completed</button>` : ""}
  ...
  if (viewBtn) viewBtn.addEventListener("click", async () => {
    const { data: wo } = await supabase
      ...
      .maybeSingle();
    close();
    if (wo) showWorkOrderDetailsModal(wo, () => renderComplianceMatrix(matrixContainer, client));
  });
  ...
  const saveDatesBtn = backdrop.querySelector("#dcm-save-dates");
  if (saveDatesBtn) saveDatesBtn.addEventListener("click", async () => {
    const errorEl = backdrop.querySelector("#dcm-dates-error");
    const reportDateText = backdrop.querySelector("#dcm-report-date").value.trim();
    const archivalDateText = backdrop.querySelector("#dcm-archival-date").value.trim();
    const reportDate = reportDateText ? dmyToISO(reportDateText) : null;
    const archivalDate = archivalDateText ? dmyToISO(archivalDateText) : null;
    if ((reportDateText && reportDate === undefined) || (archivalDateText && archivalDate === undefined)) {
      errorEl.textContent = "Please use DD/MM/YYYY for both dates.";
      errorEl.classList.remove("hidden");
      return;
    }
    const bothFilled = reportDate && archivalDate;
    const { error } = await supabase.from("client_deadlines").update({
      manual_report_date: reportDate, manual_archival_date: archivalDate,
      ...(bothFilled ? { status: "completed", completed_date: reportDate } : {}),
  ...
  const saveAmountBtn = backdrop.querySelector("#dcm-save-amount");
  if (saveAmountBtn) saveAmountBtn.addEventListener("click", async () => {
    const errorEl = backdrop.querySelector("#dcm-amount-error");
    const amountText = backdrop.querySelector("#dcm-tax-amount").value.trim();
    ...
    if (tetError) { errorEl.textContent = "Could not save: " + tetError.message; errorEl.classList.remove("hidden"); return; }
