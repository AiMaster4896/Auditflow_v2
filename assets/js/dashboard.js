...
export async function renderDashboard(el) {
  ...
  el.innerHTML = `
    ...
    <h2 style="margin-top:32px;margin-bottom:4px;">Work Order Monitoring</h2>
    ...
    <div class="tab-nav" style="display:flex;gap:4px;border-bottom:1px solid var(--gray-200);margin-bottom:20px;">
      <button type="button" class="dash-wo-tab-btn" data-dash-wo-tab="overview">Overview</button>
      <button type="button" class="dash-wo-tab-btn" data-dash-wo-tab="breakdown">Breakdown</button>
    </div>

    <div class="dash-wo-tab-panel" data-dash-wo-panel="overview">
      ...
      <div class="import-card" style="margin-top:20px;">
        <div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:10px;">
          <div>
            <h3 style="margin:0;">Deadline Calendar</h3>
            <p class="hint" style="margin-top:2px;">${isStaffView ? "Your own work order deadlines for the month." : "Every work order deadline for the month, across the firm."}</p>
          </div>
          <div style="display:flex;align-items:center;gap:10px;">
            <button type="button" id="dash-cal-prev" class="btn-secondary" style="width:auto;padding:6px 12px;">&larr;</button>
            <span id="dash-cal-label" style="font-weight:600;min-width:140px;text-align:center;"></span>
            <button type="button" id="dash-cal-next" class="btn-secondary" style="width:auto;padding:6px 12px;">&rarr;</button>
          </div>
        </div>
        <div id="dash-calendar" style="margin-top:14px;"></div>
      </div>
    </div>

    <div class="dash-wo-tab-panel" data-dash-wo-panel="breakdown" style="display:none;">
  ...
  function setActiveDashWoTab(tab) {
    el.querySelectorAll(".dash-wo-tab-btn").forEach((btn) => {
      const isActive = btn.dataset.dashWoTab === tab;
    ...
    el.querySelectorAll(".dash-wo-tab-panel").forEach((panel) => {
      panel.style.display = panel.dataset.dashWoPanel === tab ? "" : "none";
    });
  }
  el.querySelectorAll(".dash-wo-tab-btn").forEach((btn) => {
    btn.addEventListener("click", () => setActiveDashWoTab(btn.dataset.dashWoTab));
  });
  setActiveDashWoTab("overview");
  ...
  const [{ data: deadlines }, { data: members }, { data: allWO }, { data: clientsData }, { data: teamsData }, { data: branchesData }] =
    await Promise.all([
      fetchAllRows(supabase.from("client_deadlines").select("id, deadline_type, deadline_date, financial_year, status, completed_date, work_order_id, client_id, clients(legal_name, remark), work_orders(status, work_order_number, created_at, assigned_user_id)").eq("organisation_id", orgId)),
  ...
  function refreshFilteredSections() {
    ...
    renderStaffSummary();
    renderDeadlineCalendar(isStaffView);
...
function renderDeadlineCalendar(isStaffView) {
  const wrap = document.getElementById("dash-calendar");
  const label = document.getElementById("dash-cal-label");
  ...
  const selfId = getIdentity()?.user?.id;
  let relevant = dashWorkOrdersCache.filter((wo) => wo.deadline_date);
  if (isStaffView) {
    relevant = relevant.filter((wo) => wo.assigned_user_id === selfId);
  }

  const monthStr = `${calendarYear}-${String(calendarMonth + 1).padStart(2, "0")}`;
  const byDay = {};
  relevant.forEach((wo) => {
    if (!wo.deadline_date.startsWith(monthStr)) return;
    const day = parseInt(wo.deadline_date.slice(8, 10), 10);
  ...
  for (let day = 1; day <= daysInMonth; day++) {
    ...
    const dayWorkOrders = byDay[day] || [];
    const isToday = dateStr === todayStr;

    const chipsHtml = dayWorkOrders.map((wo) => {
      const isOverdue = wo.status !== "completed" && dateStr < todayStr;
      const chipColor = isOverdue ? "#fee2e2" : wo.status === "completed" ? "#dcfce7" : "#fef3c7";
      const chipText = isOverdue ? "#991b1b" : wo.status === "completed" ? "#166534" : "#92400e";
      const clientName = wo.clients?.legal_name || "Unknown Client";
      const fullLabel = `${clientName} — ${getOrderTypeLabel(wo.order_type)}`;
      return `<div class="cal-chip" data-cal-chip-client="${wo.client_id}" style="background:${chipColor};color:${chipText};" title="${fullLabel.replace(/"/g, "&quot;")}">${fullLabel}</div>`;
    }).join("");

    cellsHtml += `
      <div class="cal-cell ${isToday ? "cal-cell-today" : ""}">
        <span class="cal-day-num">${day}</span>
        <div class="cal-chip-list">${chipsHtml}</div>
      </div>
    `;
  }
  ...
  wrap.innerHTML = `
    <div class="cal-grid">
      ${dayNames.map((d) => `<div class="cal-cell cal-cell-header">${d}</div>`).join("")}
      ${cellsHtml}
    </div>
  `;

  wrap.querySelectorAll("[data-cal-chip-client]").forEach((chip) => {
    chip.addEventListener("click", () => openClientDetailsById(chip.dataset.calChipClient));
  });
}
...
function renderStatusTypeCharts() {
  const summaryGrid = document.getElementById("wo-summary-grid");
...
function showDetails(title, orders) {
  ...
  function applyWoDetailsFilters() {
    ...
    renderWorkOrdersTable(wrap, pageItems, {
      showClient: true,
      onChanged: async (savedWo) => {
        const id = savedWo?.id;
        if (id) {
          const { data: fresh } = await supabase
            ...
            .maybeSingle();
          if (fresh) {
            const replaceIn = (arr) => { const idx = arr.findIndex((w) => w.id === id); if (idx !== -1) arr[idx] = fresh; };
            replaceIn(woDetailsFullList);
            replaceIn(dashWorkOrdersCache);
          }
        }
