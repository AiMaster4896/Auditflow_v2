...
export async function renderSettings(el) {
  el.innerHTML = `
    ...
    <div class="tab-nav" style="display:flex;gap:4px;border-bottom:1px solid var(--gray-200);margin-bottom:20px;">
      <button type="button" class="settings-tab-btn" data-settings-tab="general">General</button>
      <button type="button" class="settings-tab-btn" data-settings-tab="access">Access &amp; Structure</button>
      <button type="button" class="settings-tab-btn" data-settings-tab="workorders">Work Orders</button>
      <button type="button" class="settings-tab-btn" data-settings-tab="templates">Workspace &amp; Templates</button>
    </div>
    ...
    <div class="settings-tab-panel" data-settings-panel="access" style="display:none;">
      <div class="import-card">
        ...
        <label style="display:flex;align-items:center;gap:8px;font-size:14px;margin-top:16px;">
          <input type="checkbox" id="elearn-enabled-toggle" /> Enable E-Learn module
        </label>
        <p class="hint" style="margin-top:6px;">When turned off, E-Learn disappears from the sidebar for staff and managers. Admins keep access so they can continue building courses while it's turned off for everyone else.</p>
    ...
    <div class="settings-tab-panel" data-settings-panel="workorders" style="display:none;">
      <div class="import-card">
    ...
    <div class="settings-tab-panel" data-settings-panel="templates" style="display:none;">
      <div class="import-card">
  ...
  function setActiveSettingsTab(tab) {
    ...
    el.querySelectorAll(".settings-tab-panel").forEach((panel) => {
      panel.style.display = panel.dataset.settingsPanel === tab ? "" : "none";
    });
  }
  el.querySelectorAll(".settings-tab-btn").forEach((btn) => {
  ...
  const [{ data: org }, { data: orgSettings }] = await Promise.all([
    ...
    supabase.from("organisation_settings").select("logo_url, workspace_background_url, independence_template_url, announcement_text, banner_mode, client_won_template, client_lost_template, staff_client_list_access, elearn_enabled").eq("organisation_id", orgId).maybeSingle(),
  ]);
  ...
  document.getElementById("elearn-enabled-toggle").checked = orgSettings?.elearn_enabled ?? true;
  document.getElementById("elearn-enabled-toggle").addEventListener("change", async (e) => {
    const errorEl = document.getElementById("staff-permissions-error");
    const { error } = await supabase.from("organisation_settings").update({ elearn_enabled: e.target.checked }).eq("organisation_id", orgId);
    if (error) {
      errorEl.textContent = "Could not save: " + error.message;
      errorEl.classList.remove("hidden");
      e.target.checked = !e.target.checked;
